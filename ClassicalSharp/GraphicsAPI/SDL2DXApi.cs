...
namespace ClassicalSharp.GraphicsAPI {
	...
	public unsafe class SDL2DXApi : IGraphicsApi {
		...
		IntPtr GetWinHandle(Game game) {
			// The WindowInfo stored in SDL2Window holds an IntPtr to the SDL_Window, but using it, we can
			// get the HWND associated with the window, which is what DirectX wants.
			SDL.SDL_SysWMinfo wmInfo = new SDL.SDL_SysWMinfo();
			
			SDL.SDL_GetWindowWMInfo(game.window.WindowInfo.WinHandle, ref wmInfo);
			
			if (wmInfo.subsystem != SDL.SDL_SYSWM_TYPE.SDL_SYSWM_WINDOWS) {
				throw new InvalidOperationException("DirectX shouldn't be used outside of Windows (or Wine).");
			}
			
			return wmInfo.info.win.window;
		}
		
		public SDL2DXApi(Game game) {
			MinZNear = 0.05f;
			IntPtr windowHandle = GetWinHandle(game);
			d3d = new Direct3D();
